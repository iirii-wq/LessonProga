...
def run_game():
    ...
    print("-" * 45)
    print(f'Начинаем зачистку уровня! Впереди {len(monsters)} монстра')
    print("-" * 45)
    ...
    for monster in monsters:
        ...
        while monster.is_alive() and hunter.is_alive():
            weapon_index = (turn % 3) + 1
            hunter.attack(weapon_index, monster)

            if not monster.is_alive():
                # Сообщение уже выведено в hunter.attack()
                break
    ...
    print("-" * 45)
    if hunter.is_alive():
        print("!!!Победа!!! Уровень полностью зачищен от нечисти.")
    else:
